import { z } from 'zod';
import { parseCidr } from '../../common/auth/ip.js';
...
const cidrSchema = z.string().trim().min(1).max(64).superRefine((value, context) => {
  try {
    parseCidr(value);
  } catch {
    context.addIssue({ code: z.ZodIssueCode.custom, message: 'cidr must be a valid IPv4 or IPv6 CIDR' });
  }
}).transform((value) => parseCidr(value));

export const ipAllowlistSchema = z.object({ label: z.string().trim().min(1).max(160), cidr: cidrSchema, scope: z.enum(['ALL', 'ROLE', 'USER']).default('ALL'), roleId: z.string().uuid().optional(), userId: z.string().uuid().optional(), expiresAt: z.string().datetime({ offset: true }).optional() });
