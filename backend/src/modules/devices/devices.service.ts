...
export class DevicesService {
  ...
  async revoke(id: string, actor: Pick<AccessTokenClaims, 'sub' | 'role'>, reason = 'ADMIN_REVOKE') {
    const [row] = await this.db.db.update(devices).set({ status: 'REVOKED', revokedAt: new Date(), revokedReason: reason, tokenHash: null }).where(eq(devices.id, id)).returning({ id: devices.id, status: devices.status, revokedAt: devices.revokedAt });
