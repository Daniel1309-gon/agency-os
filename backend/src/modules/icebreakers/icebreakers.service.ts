...
export class IcebreakersService {
  ...
  async list(operatorId: string) { return this.db.db.select({ id: icebreakers.id, profileId: icebreakers.profileId, text: icebreakers.text, status: icebreakers.status, version: icebreakers.version, publishedAt: icebreakers.publishedAt, createdAt: icebreakers.createdAt, updatedAt: icebreakers.updatedAt }).from(icebreakers).where(eq(icebreakers.operatorId, operatorId)).orderBy(asc(icebreakers.createdAt)); }

  async create(input: IcebreakerCreateInput, operatorId: string) { const [row] = await this.db.db.insert(icebreakers).values({ ...input, operatorId }).returning({ id: icebreakers.id, status: icebreakers.status, version: icebreakers.version }); return row; }

  async update(id: string, input: IcebreakerUpdateInput, operatorId: string) {
    const { version, ...changes } = input;
    const [row] = await this.db.db.update(icebreakers).set({ ...changes, version: version + 1, updatedAt: new Date() }).where(and(eq(icebreakers.id, id), eq(icebreakers.operatorId, operatorId), eq(icebreakers.version, version), sql`${icebreakers.status} IN ('DRAFT', 'BLOCKED')`)).returning({ id: icebreakers.id, version: icebreakers.version, status: icebreakers.status });
    if (!row) throw new ConflictException('Icebreaker is not editable or has changed');
    return row;
  }

  async evaluate(id: string, operatorId: string) {
    const item = await this.db.db.query.icebreakers.findFirst({ where: and(eq(icebreakers.id, id), eq(icebreakers.operatorId, operatorId)) });
  ...
  async evaluations(id: string, operatorId: string) { const item = await this.db.db.select({ id: icebreakers.id }).from(icebreakers).where(and(eq(icebreakers.id, id), eq(icebreakers.operatorId, operatorId))).limit(1); if (!item.length) throw new NotFoundException('Icebreaker not found'); return this.db.db.select().from(icebreakerEvaluations).where(eq(icebreakerEvaluations.icebreakerId, id)).orderBy(asc(icebreakerEvaluations.createdAt)); }
  async violations(filters: { from?: string; to?: string; operatorId?: string }) { return this.db.db.select({ id: icebreakerViolations.id, icebreakerId: icebreakerViolations.icebreakerId, ruleId: icebreakerViolations.ruleId, severity: icebreakerViolations.severity, status: icebreakerViolations.status, reviewedBy: icebreakerViolations.reviewedBy, reviewedAt: icebreakerViolations.reviewedAt, createdAt: icebreakerViolations.createdAt }).from(icebreakerViolations).innerJoin(icebreakers, eq(icebreakers.id, icebreakerViolations.icebreakerId)).where(and(filters.operatorId ? eq(icebreakers.operatorId, filters.operatorId) : undefined, filters.from ? sql`${icebreakerViolations.createdAt} >= ${new Date(filters.from)}` : undefined, filters.to ? sql`${icebreakerViolations.createdAt} < ${new Date(filters.to)}` : undefined)).orderBy(asc(icebreakerViolations.createdAt)); }
  async effectiveness(id: string, operatorId: string) { const item = await this.db.db.select({ id: icebreakers.id }).from(icebreakers).where(and(eq(icebreakers.id, id), eq(icebreakers.operatorId, operatorId))).limit(1); if (!item.length) throw new NotFoundException('Icebreaker not found'); return this.db.db.select().from(icebreakerEffectiveness).where(eq(icebreakerEffectiveness.operatorId, operatorId)).orderBy(asc(icebreakerEffectiveness.businessDate)); }

  async review(id: string, input: ReviewInput, reviewerId: string) { const item = await this.db.db.query.icebreakers.findFirst({ where: eq(icebreakers.id, id) }); if (!item) throw new NotFoundException('Icebreaker not found'); const [row] = await this.db.db.insert(icebreakerReviews).values({ icebreakerId: id, reviewerId, verdict: input.verdict, reason: input.reason }).returning({ id: icebreakerReviews.id, verdict: icebreakerReviews.verdict }); if (input.verdict === 'OVERRIDDEN_ALLOW') await this.db.db.update(icebreakers).set({ status: 'APPROVED', updatedAt: new Date(), version: item.version + 1 }).where(eq(icebreakers.id, id)); return row; }
}
