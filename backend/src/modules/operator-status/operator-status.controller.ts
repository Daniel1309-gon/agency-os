...
export class OperatorStatusController {
  ...
  @Post('me/status') @UsePipes(new ZodValidationPipe(operatorStatusSchema)) set(@Body() body: OperatorStatusInput, @CurrentUser() user: AccessTokenClaims) { return this.status.set(user.sub, body); }
}
