...
export const shiftOverrideSchema = z.object({ operatorId: z.string().uuid(), validFrom: z.string().datetime({ offset: true }), validTo: z.string().datetime({ offset: true }), type: z.enum(['OVERTIME', 'EXTENDED_SHIFT', 'SPECIAL_PERMISSION']), reason: z.string().trim().min(1).max(1000) }).refine((value) => new Date(value.validFrom).getTime() < new Date(value.validTo).getTime(), 'validTo must be after validFrom');
export const effectiveTimeQuerySchema = z.object({ from: z.string().date(), to: z.string().date(), operatorId: z.string().uuid().optional(), crewId: z.string().uuid().optional(), page: z.coerce.number().int().min(1).default(1), pageSize: z.coerce.number().int().min(1).max(100).default(25) });
