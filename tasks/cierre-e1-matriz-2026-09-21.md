...
| SEC-07 escritor central de auditoría | `PARCIAL` | `common/audit/audit.service.ts` (whitelist fail-closed); atomicidad vía `TransactionInterceptor`; caminos sin JWT cerrados con transacción propia (SEC-07b: login, refresh, enroll, webhook y rutas de estación) con prueba de auditoría que falla → no queda la escritura | Falta el catálogo por acción (SEC-07a) | E1-07a |
