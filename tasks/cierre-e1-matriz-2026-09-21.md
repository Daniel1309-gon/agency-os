...
| SEC-08 partición, inmutabilidad y retención | `CERRADO` (2026-09-23) | Migración `0016`, `schema-invariants.int.spec.ts`; `audit.retention_months=2` (migración `0023`, ~59–92 días); `GET /admin/audit-log` con cursor y prueba de páginas sin huecos ni duplicados con timestamps iguales (`shifts-and-crews.int.spec.ts`, «pages the audit log with a stable cursor») | — | Conservar como regresión |
...
| Requisito | Estado | Evidencia actual | Brecha concreta | Prueba necesaria |
|---|---|---|---|---|
| OPS-01 CRUD de perfiles | `CERRADO` (2026-09-23) | `profiles.controller.ts`, `profiles.service.ts`: versión optimista en el cuerpo (`WHERE version = $n`, no ETag) y auditoría de alta/cambio/baja; versión vieja → 409 sin pisar el dato nuevo (`shifts-and-crews.int.spec.ts`, bloque `ProfilesService`) | — | Conservar como regresión |
