...
| **FR-04** Operador solo en turno y horas extra | No cumple / P0 | Existen `shifts`, `shift_overrides` y endpoints de inicio/fin. | No existe `ShiftWindowGuard`; login, vault, sesiones, métricas, pedidos e icebreakers funcionan fuera de turno. | Operador no obtiene ni renueva sesión fuera de un `shift`/override válido; toda acción operativa revalida la ventana; ADMIN/DIRECTOR/COORDINADOR/CAFETERIA usan reglas explícitas. Bordes semiabiertos `[)` y horarios 06:05/14:05/22:05 probados. | SEC-06, OPS-05 |
...
- **Trabajo:** servicio `ShiftAccessPolicy`; guard/decorator para operaciones; check dentro de login/refresh para OPERADOR; overrides aprobados; bordes `[)`; lista explícita de acciones no operativas si negocio decide permitir alguna.
