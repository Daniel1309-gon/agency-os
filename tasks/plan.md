...
| **FR-04** Operador solo en turno y horas extra | Parcial / P0 | `ShiftAccessService`, `ShiftWindowGuard`, checks de login/refresh, rangos `[)` y overrides revocables; pruebas unitarias e integración PostgreSQL en `backend/src/test/integration/shift-access.int.spec.ts`. | La política ya deniega fuera de ventana en las superficies decoradas, pero la materialización/cierre automático de turnos, relevos y sesiones todavía pertenece a OPS-05/OPS-03; el alcance RBAC/RLS completo sigue condicionado por SEC-02. | Operador no obtiene ni renueva sesión fuera de un `shift`/override válido; toda acción operativa revalida la ventana; ADMIN/DIRECTOR/COORDINADOR/CAFETERIA usan reglas explícitas. Bordes semiabiertos `[)` y horarios 06:05/14:05/22:05 probados. | SEC-06, OPS-03, OPS-05 |
