...
- [ ] **OPS-03** Máquina de sesión, CAS, reaper y gracia 0. Subavance verificado: `end()` cierra transaccionalmente sesiones `LAUNCHING/ACTIVE/ERROR`, el reaper cierra esos tres estados al expirar la asignación y el relevo usa `SHIFT_ENDED` en el borde exacto; evidencia en `backend/src/modules/assignments/assignments.service.ts`, `backend/src/modules/jobs/jobs.service.ts`, `backend/src/test/integration/assignments.int.spec.ts` y [CI run 32741434728](https://github.com/Daniel1309-gon/agency-os/actions/runs/32741434728). Pendiente CAS/versionado explícito, heartbeat/device/chrome-profile completo, `STALE` y reaper durable.
