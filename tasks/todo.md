...
- [ ] **SEC-11** `JwtAuthGuard` traduce fallos de infraestructura a `401`. La consulta de usuario activo vive dentro del `try` con un `catch` desnudo (`backend/src/common/auth/guards.ts:66-75`), asi que una caida de Postgres devuelve `401 Invalid or expired token` y escribe una fila `INVALID_TOKEN` falsa en `audit_log` — justo la evidencia de la que dependen SEC-02 y SEC-10, con lo que un incidente de infraestructura queda indistinguible de un ataque. Corregir sacando la consulta del `try` o relanzando lo que no sea error de verificacion de JWT. Hallazgo de la revision del 2026-09-08: [`code-review-2026-09-08.md`](evidence/code-review-2026-09-08.md).
